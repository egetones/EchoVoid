...
1.  **Encapsulation:** EchoVoid intercepts the user's message.
2.  **Injection:** The message is injected into the payload of a standard Ping packet.
3.  **Transmission:** The packet traverses the network as a legitimate diagnostic request.
4.  **Extraction:** The listener captures the packet, discards the header, and reconstructs the hidden message.
...
```bash
git clone https://github.com/egetones/EchoVoid.git
cd EchoVoid
```
